...
impl StorageProvider for LocalDesktopProvider {
...
mod tests {
    ...
    fn make_provider() -> LocalDesktopProvider {
        LocalDesktopProvider::new(&LocalConfig::default())
    }

    #[tokio::test]
    async fn test_local_desktop_fetcher_fetch_success() {
        let mut tmp = NamedTempFile::new().unwrap();
        writeln!(tmp, "hello local desktop").unwrap();
        let path = format!("local://{}", tmp.path().display());

        let provider = make_provider();
        let result = provider.fetch(&make_auth(), &path).await.unwrap();

        assert!(String::from_utf8(result)
            .unwrap()
            .contains("hello local desktop"));
    }

    #[tokio::test]
    async fn test_local_desktop_fetcher_fetch_missing_file() {
        let provider = make_provider();
        let bad_path = "local:///definitely/missing/file";
    ...
    #[tokio::test]
    async fn test_local_desktop_fetcher_size_and_mtime_success() {
        let mut tmp = NamedTempFile::new().unwrap();
        write!(tmp, "123456").unwrap();
        let path = format!("local://{}", tmp.path().display());
    ...
    #[tokio::test]
    async fn test_local_desktop_fetcher_unsupported_uri() {
        let provider = make_provider();
        ...
        assert!(result
            .unwrap_err()
            .message
            .contains("unsupported local URI"));
    }

    #[tokio::test]
    async fn test_local_desktop_fetcher_empty_path() {
        let provider = make_provider();
        let result = provider.fetch(&make_auth(), "local://").await;
        assert!(result.is_err());
        assert!(result
            .unwrap_err()
            .message
            .contains("missing a filesystem path"));
    }

    #[tokio::test]
    async fn test_local_desktop_fetcher_size_and_mtime_missing_file() {
        let provider = make_provider();
        let bad_path = "local:///definitely/missing/file";
    ...
    #[tokio::test]
    async fn test_local_desktop_fetcher_size_and_mtime_unsupported_uri() {
        let provider = make_provider();
        let result = provider
            .size_and_mtime(&make_auth(), "unsupported://path")
            .await;
        assert!(result.is_err());
        assert!(result
            .unwrap_err()
    ...
    #[tokio::test]
    async fn test_local_desktop_fetcher_size_and_mtime_empty_path() {
        let provider = make_provider();
        let result = provider.size_and_mtime(&make_auth(), "local://").await;
        assert!(result.is_err());
        assert!(result
            .unwrap_err()
            .message
    ...
    #[tokio::test]
    async fn test_local_desktop_fetcher_save_success() {
    ...
    #[tokio::test]
    async fn test_local_desktop_fetcher_save_unsupported_uri() {
        let provider = make_provider();
        let result = provider
            .save(&make_auth(), "unsupported://path", b"data")
            .await;
        assert!(result.is_err());
        assert!(result
            .unwrap_err()
    ...
    #[tokio::test]
    async fn test_local_desktop_fetcher_save_empty_path() {
        let provider = make_provider();
        let result = provider.save(&make_auth(), "local://", b"data").await;
        assert!(result.is_err());
        assert!(result
            .unwrap_err()
            .message
    ...
    #[tokio::test]
    async fn test_local_desktop_fetcher_save_with_parent_directories() {
        let dir = TempDir::new().unwrap();
        let file_path = dir.path().join("subdir").join("file.txt");
        let path = format!("local://{}", file_path.display());
        let data = b"test data in subdirectory";
    ...
    #[tokio::test]
    async fn test_local_desktop_fetcher_resolves_relative_with_base_path() {
        let dir = TempDir::new().unwrap();
        let config = LocalConfig {
            base_path: Some(dir.path().to_string_lossy().into_owned()),
        };
        let provider = LocalDesktopProvider::new(&config);
        let relative_path = "local://nested/file.txt";
        let data = b"content";

        provider
            .save(&make_auth(), relative_path, data)
            .await
            .unwrap();
