...
8. **古いContent削除**: 古いContentIdのContentを削除（失敗しても問題なし）
   - InMemory実装の場合: プロセス終了時にメモリが解放される
   - Sled実装の場合: 明示的に削除しない限り残るが、参照されないため問題ない
9. **古いCEK削除**: 古いContentIdのCEKを削除（失敗しても問題なし）
   - InMemory実装の場合: プロセス終了時にメモリが解放される
   - Sled実装の場合: 明示的に削除しない限り残るが、参照されないため問題ない
10. **結果返却**: 暗号化されたコンテンツ（ciphertext）を含む`ReencryptContentResult`を返す

**注意**: 削除操作（Step 8、9）はアトミック性を保証しない。保存操作（Step 6、7）が成功すれば、削除操作が失敗しても問題ない。古いContentIdのデータ（ContentとCEK）は参照されないため、実質的に無効となる。
...
async fn reencrypt_content(
    ...
    let requester_key_bytes = BASE64_STANDARD
        .decode(&req.requester_key_id_base64)
        .map_err(|e| {
            (
                StatusCode::BAD_REQUEST,
                format!("invalid requester_key_id_base64: {e}"),
            )
        })?;
    let requester_key_id = KeyId::new(requester_key_bytes);
    
    // revoked_key_id_base64をbase64デコードしてKeyIdに変換
    let revoked_key_bytes = BASE64_STANDARD
        .decode(&req.revoked_key_id_base64)
        .map_err(|e| {
            (
                StatusCode::BAD_REQUEST,
                format!("invalid revoked_key_id_base64: {e}"),
            )
        })?;
    let revoked_key_id = KeyId::new(revoked_key_bytes);
    ...
    let result = state
        ...
        .map_err(|e| {
            let status = match e {
                ...
                ReencryptError::RevokedKeyIdStillExists { .. } => StatusCode::BAD_REQUEST,
...
##### 8. 古いContentIdのContentを削除（アプリケーションサービス層）

- **インフラストラクチャ層（ポート）**: `ContentRepository::delete(old_content_id)` を呼び出し
- **エラー処理**: 削除が失敗しても問題なし
  - InMemory実装の場合: プロセス終了時にメモリが解放される
  - Sled実装の場合: 明示的に削除しない限り残るが、参照されないため問題ない
- **注意**: 削除操作はアトミック性を保証しない。保存操作（Step 6、7）が成功すれば、削除操作が失敗しても問題ない

##### 9. 古いContentIdのCEKを削除（アプリケーションサービス層）
...
**保存操作のアトミック性保証**: Step 6とStep 7の保存操作については、アトミック性を保証する。Content保存が失敗した場合、新しいContentIdのCEKを削除してロールバックする。

**削除操作について**: Step 8とStep 9の削除操作はアトミック性を保証しない。保存操作が成功すれば、削除操作が失敗しても問題ない。古いContentIdのデータ（ContentとCEK）は参照されないため、実質的に無効となる。
  - InMemory実装の場合: プロセス終了時にメモリが解放される
  - Sled実装の場合: 明示的に削除しない限り残るが、参照されないため問題ない
...
pub fn reencrypt(&self, cmd: ReencryptContentCommand) -> Result<ReencryptContentResult, ReencryptError> {
    ...
    // Step 8: 古いContentIdのContentを削除（失敗しても問題なし、メモリ解放やGCのタイミングで消える）
    let _ = self.content_repository.delete(&old_content_id);
    
    // Step 9: 古いContentIdのCEKを削除（失敗しても問題なし、メモリ解放やGCのタイミングで消える）
    let _ = self.cek_store.delete(&old_content_id);
    
    // 保存操作が成功: 新しいContentIdで保存されたデータ（ContentとCEK）が残る
...
- 新しいContentIdでCEKとContentを保存し、Content保存が失敗した場合、新しいContentIdのCEKを削除してロールバック
- これにより、「CEKの生成はできたけどContent保存はうまくできなかった」などの整合性を満たせない状態が発生しない

**削除操作について**:
- 削除操作（古いContentIdのContentとCEKの削除）はアトミック性を保証しない
...
2. monas-state-node側で削除確認を実施（revoked_key_idがShareに存在しないことを確認）
   -> 成功: Step 3へ
   -> 失敗: エラーを返す（400 Bad Request）
3. ContentService::reencrypt()を呼び出す
   -> 権限チェック済みのリクエストとして処理
```
...
- `ReencryptError::RevokedKeyIdStillExists`: 削除確認は外部で実施する場合、不要
