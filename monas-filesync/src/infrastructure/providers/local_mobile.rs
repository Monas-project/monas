...
mod tests {
    ...
    async fn test_local_mobile_fetcher_size_and_mtime_empty_path() {
        let provider = make_provider();
        let result = provider
            .size_and_mtime(&make_auth(), "local-mobile://")
            .await;
        assert!(result.is_err());
        assert!(result
            .unwrap_err()
    ...
    async fn test_local_mobile_fetcher_save_empty_path() {
        let provider = make_provider();
        let result = provider
            .save(&make_auth(), "local-mobile://", b"data")
            .await;
        assert!(result.is_err());
        assert!(result
            .unwrap_err()
    ...
    #[tokio::test]
    async fn test_local_mobile_rejects_parent_dir_double_dot() {
        let provider = make_provider();
        let malicious_path = "local-mobile://content/../../../etc/passwd";
    ...
    #[tokio::test]
    async fn test_local_mobile_rejects_parent_dir_in_middle() {
        let provider = make_provider();
        let malicious_path = "local-mobile://foo/../bar/../../../etc/passwd";
    ...
    #[tokio::test]
    async fn test_local_mobile_rejects_parent_dir_on_save() {
        let provider = make_provider();
        let malicious_path = "local-mobile://content/../../../tmp/evil.txt";
    ...
    #[tokio::test]
    async fn test_local_mobile_allows_valid_nested_path() {
        let dir = TempDir::new().unwrap();
        let config = LocalConfig {
            base_path: Some(dir.path().to_string_lossy().into_owned()),
        };
        ...
        let valid_path = "local-mobile://content/subdir/file.txt";
    ...
    fn test_has_parent_dir_detects_double_dot() {
        assert!(LocalMobileProvider::has_parent_dir("../foo"));
        assert!(LocalMobileProvider::has_parent_dir("foo/../bar"));
        assert!(LocalMobileProvider::has_parent_dir("foo/bar/.."));
        assert!(LocalMobileProvider::has_parent_dir(".."));
    }

    #[test]
    fn test_has_parent_dir_allows_valid_paths() {
        assert!(!LocalMobileProvider::has_parent_dir("foo/bar"));
        assert!(!LocalMobileProvider::has_parent_dir("content/file.txt"));
        assert!(!LocalMobileProvider::has_parent_dir("a.b.c/d.e.f"));
        assert!(!LocalMobileProvider::has_parent_dir(".hidden/file"));
    }
}
