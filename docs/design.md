...
残る課題は**member discoveryそのもの**である。`ContentNetwork`レコードは`ContentCreated`イベント由来だが、そのイベント自体に署名検証は無い。「このノードが本当にこのcontentのmemberである」ことを暗号学的に確認する仕組みは未実装で、attested / 未証明の区別はそこへ至るまでの近似にとどまる。
