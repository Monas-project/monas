...
リクエストの同一性には**リクエスト署名そのもののdigest**を使う。署名は既に操作・リソース・timestamp・body digestすべてに束縛されているので、digestが一致する＝完全に同じリクエストの再送であり、nonceのような新しいフィールドをワイヤ形式へ足す必要がない。記録の保持期間は鮮度窓と同じでよい（窓の外へ出た署名は記録が無くても鮮度チェックで落ちる）ため、記録は無制限には育たない。読み取りは冪等なのでこの記録の対象外である。

したがってTokenはTTL内で何度でも再利用できる一方、**個々のリクエスト署名は使い切り**である。記録はノードごとに独立で、同じ署名を複数のレプリカへ送ればそれぞれで1回ずつ受理される（CRDTは同一操作の重複適用に耐えるが、ネットワーク全体で厳密に1回を保証するものではない）。またデフォルト実装はプロセス内に閉じており、再起動をまたいで5分以内に届いた同一署名までは防げない。JWT自体の署名検証は、受信したワイヤ上のバイト列（`header.payload`セグメント）に対して行う。
...
| 由来 | 内容 | 扱い |
|---|---|---|
| ローカルの`ContentNetwork`レコード | `ContentCreated`イベント由来。ネットワークが実際に割り当てたノード | attested |
| DHTの近傍探索 | `sha256(content_id)`に近いというだけ。コンテンツとの関連は何も示されていない | 未証明 |

**未証明の候補が返した401/403は権威として扱わない。** 権威にすると、DHTキーの近くにPeer IDを置いた1台が403を返すだけであらゆるread/writeを止められてしまう（可用性への攻撃）。また正規のmemberであっても、policyの複製が終わっていない部分同期状態なら403を返し得るため、健全なレプリカへのfailoverを潰さないためにも継続が必要である。ただし答えとしては保持し、他の候補から何も得られなければそれを返す。attestedなmemberの401/403は実policyに対する評価結果なので、そこで打ち切る。

**未証明の候補へcredentialを渡す数には上限を設ける。** relayはcallerのtokenとリクエスト署名をそのまま転送するため、候補を1台試すごとに、コンテンツとの関連が何も示されていない相手へ生きた認証情報を開示することになる。候補リスト全体を舐めてはならない。

なお、この開示自体を無くすには「relay先・操作・リソースに限定した短命なケイパビリティへ交換する」プロトコル上の仕組みが要る。現状はattested / 未証明の区別と開示数の上限による緩和にとどまり、未証明の候補が1台もcredentialを見ないことは保証していない。
