...
NODE4_ID=$(echo "$NODE4_INFO" | jq -r '.node_id')
...
test_request "ノード4の登録" POST "http://127.0.0.1:8083/node/register" '{"total_capacity": 1800000}'
...
test_request "ノード4から見たノード一覧" GET "http://127.0.0.1:8083/nodes"
...
echo "  - tail -f logs/node4.log | grep -E 'Peer|connected|discovered'"
